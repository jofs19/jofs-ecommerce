    ...
    <form method="post" action="{{ route('vendor.profile.store') }}" enctype="multipart/form-data">
      ...
      <div class="bg-secondary rounded-3 p-4 mb-4 mr-4 pr-4">
        <div class="d-flex align-items-center"><img class="rounded"
          src="{{ (!empty($vendorData->profile_photo_path)) ? asset($vendorData->profile_photo_path):url('upload/no_image.jpg') }}" width="90" alt="User">
          <div class="ps-3">
      ...
      <div class="row gx-4 gy-3">
        <div class="col-sm-6">
          ...
          <input class="form-control" type="text" id="account-fn" value="{{ $vendorData->username }}" style="width: 22rem;">
        </div>
        {{-- <div class="col-sm-5">
          <label class="form-label" for="account-ln">Email Address</label>
          <input class="form-control" name="email" type="email" id="account-ln" value="{{ $vendorData->email }}" disabled>
          @error('email')
        ...
        <div class="col-sm-5">
          ...
          <input class="form-control" name="phone" type="text" id="account-phone" value="{{ $vendorData->phone }}" style="width: 22rem;" required>
        </div>
